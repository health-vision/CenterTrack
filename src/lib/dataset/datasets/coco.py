...
class COCO(GenericDataset):
    ...
    def __init__(self, opt, split):
        ...
        if opt.trainval:
            split = "test"
            ann_path = os.path.join(data_dir, "annotations", "image_info_test-dev2017.json")
        else:
    ...
    def save_results(self, results, save_dir):
        json.dump(self.convert_eval_format(results), open("{}/results_coco.json".format(save_dir), "w"))
