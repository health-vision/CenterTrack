...
class COCOHP(GenericDataset):
    ...
    def __init__(self, opt, split):
        ...
        if split == "test":
            ann_path = os.path.join(data_dir, "annotations", "image_info_test-dev2017.json").format(split)
        else:
            ann_path = os.path.join(data_dir, "annotations", "person_keypoints_{}2017.json").format(split)
    ...
    def save_results(self, results, save_dir):
        json.dump(self.convert_eval_format(results), open("{}/results_cocohp.json".format(save_dir), "w"))
