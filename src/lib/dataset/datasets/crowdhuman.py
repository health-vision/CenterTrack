...
class CrowdHuman(GenericDataset):
        ...
        for image_id in all_bboxes:
            if type(all_bboxes[image_id]) != type({}):
                ...
                for j in range(len(all_bboxes[image_id])):
                    ...
                    detection = {"tag": 1, "box": bbox_out, "score": float("{:.2f}".format(item["score"]))}
    ...
    def save_results(self, results, save_dir):
        self._save_results(self.convert_eval_format(results), "{}/results_crowdhuman.odgt".format(save_dir))
