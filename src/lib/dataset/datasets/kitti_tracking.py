...
class KITTITracking(GenericDataset):
    ...
    def __init__(self, opt, split):
        ...
        img_dir = os.path.join(data_dir, "data_tracking_image_2", "{}ing".format(split_), "image_02")
        ann_file_ = split_ if opt.dataset_version == "" else opt.dataset_version
        print("Warning! opt.dataset_version is not set")
        ann_path = os.path.join(data_dir, "annotations", "tracking_{}.json".format(ann_file_))
        self.images = None
            ...
            for image_info in images:
                ...
                for i in range(len(results[img_id])):
                    ...
                    if not ("rot_y" in item):
                        item["rot_y"] = -10
                    if "dim" in item:
                        item["dim"] = [max(item["dim"][0], 0.01), max(item["dim"][1], 0.01), max(item["dim"][2], 0.01)]
                    if not ("dim" in item):
                        item["dim"] = [-1, -1, -1]
                    if not ("loc" in item):
                    ...
                    f.write(" {:d} {:d} {:d}".format(int(item["dim"][0]), int(item["dim"][1]), int(item["dim"][2])))
                    f.write(" {:d} {:d} {:d}".format(int(item["loc"][0]), int(item["loc"][1]), int(item["loc"][2])))
