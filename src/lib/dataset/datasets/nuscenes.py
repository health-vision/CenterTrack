...
class nuScenes(GenericDataset):
    ...
    def __init__(self, opt, split):
        ...
        img_dir = os.path.join(data_dir, "v1.0-mini" if split_name == "mini" else "v1.0-trainval")
    ...
    def convert_eval_format(self, results):
        ...
        for image_id in self.images:
            ...
            for item in results[image_id]:
                ...
                if "size" in item:
                    ...
                    size = [float(item["dim"][1]), float(item["dim"][2]), float(item["dim"][0])]
                if "translation" in item:
                    translation = item["translation"]
                else:
                    translation = np.dot(
                        trans_matrix,
                        np.array([item["loc"][0], item["loc"][1] - size[2], item["loc"][2], 1], np.float32),
                    )
                ...
                if not ("rotation" in item):
                    ...
                    loc = np.array([item["loc"][0], item["loc"][1], item["loc"][2]], np.float32)
                ...
                nuscenes_att = (
                    np.array(item["nuscenes_att"], np.float32) if "nuscenes_att" in item else np.zeros(8, np.float32)
                )
                att = ""
                ...
                if "velocity" in item and len(item["velocity"]) == 2:
                    ...
                    velocity = np.dot(trans_matrix, np.array([velocity[0], velocity[1], velocity[2], 0], np.float32))
        ...
        for sample_token in ret["results"].keys():
            confs = sorted([(-d["detection_score"], ind) for ind, d in enumerate(ret["results"][sample_token])])
            ret["results"][sample_token] = [
                ret["results"][sample_token][ind] for _, ind in confs[: min(500, len(confs))]
            ]

        return ret

    def save_results(self, results, save_dir, task):
        json.dump(self.convert_eval_format(results), open("{}/results_nuscenes_{}.json".format(save_dir, task), "w"))
