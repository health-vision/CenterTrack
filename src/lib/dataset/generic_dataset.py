...
class GenericDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if opt.tracking:
            pre_image, pre_anns, frame_dist = self._load_pre_data(
                img_info["video_id"], img_info["frame_id"], img_info["sensor_id"] if "sensor_id" in img_info else 1
            )
            if flipped:
        ...
        for k in range(num_objs):
            ...
            if cls_id <= 0 or ("iscrowd" in ann and ann["iscrowd"] > 0):
                ...
                continue
            self._add_instance(
                ret, gt_det, k, cls_id, bbox, bbox_amodal, ann, trans_output, aug_s, calib, pre_cts, track_ids
            )
    ...
    def _get_pre_dets(self, anns, trans_input, trans_output):
        ...
        for ann in anns:
            cls_id = int(self.cat_ids[ann["category_id"]])
            if cls_id > self.opt.num_classes or cls_id <= -99 or ("iscrowd" in ann and ann["iscrowd"] > 0):
                continue
        ...
        for k in range(len(anns)):
            ...
            if "hps" in self.opt.heads and "keypoints" in anns[k]:
                ...
                for e in self.flip_idx:
                    keypoints[e[0]], keypoints[e[1]] = keypoints[e[1]].copy(), keypoints[e[0]].copy()
    ...
    def _init_ret(self, ret, gt_det):
        ...
        ret["hm"] = np.zeros((self.opt.num_classes, self.opt.output_h, self.opt.output_w), np.float32)
    ...
    def _get_bbox_output(self, bbox, trans_output, height, width):
        ...
        rect = np.array(
            [[bbox[0], bbox[1]], [bbox[0], bbox[3]], [bbox[2], bbox[3]], [bbox[2], bbox[1]]], dtype=np.float32
        )
        for t in range(4):
    ...
    def _add_instance(
        self, ret, gt_det, k, cls_id, bbox, bbox_amodal, ann, trans_output, aug_s, calib, pre_cts=None, track_ids=None
    ):
        h, w = bbox[3] - bbox[1], bbox[2] - bbox[0]
        if h <= 0 or w <= 0:
            return
        ...
        gt_det["bboxes"].append(
            np.array([ct[0] - w / 2, ct[1] - h / 2, ct[0] + w / 2, ct[1] + h / 2], dtype=np.float32)
        )
        ...
        for j in range(num_joints):
            ...
            if pts[j, 2] > 0:
                if (
                    ...
                    ret["hp_ind"][k * num_joints + j] = pt_int[1] * self.opt.output_w + pt_int[0]
