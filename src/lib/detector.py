...
class Detector(object):
    def __init__(self, opt):
        ...
        self.rest_focal_length = (
            self.trained_dataset.rest_focal_length if self.opt.test_focal_length < 0 else self.opt.test_focal_length
        )
    ...
    """
        ...
        for det in dets:
            ...
            if h > 0 and w > 0:
                ...
                ct_out = np.array([(bbox_out[0] + bbox_out[2]) / 2, (bbox_out[1] + bbox_out[3]) / 2], dtype=np.int32)
    ...
    def _get_default_calib(self, width, height):
        calib = np.array(
            [[self.rest_focal_length, 0, width / 2, 0], [0, self.rest_focal_length, height / 2, 0], [0, 0, 1, 0]]
        )
        return calib
    ...
    def debug(self, debugger, images, dets, output, scale=1, pre_images=None, pre_hms=None):
        ...
        if pre_images is not None:
            ...
            pre_img = np.clip(((pre_img * self.std + self.mean) * 255.0), 0, 255).astype(np.uint8)
    ...
    def show_results(self, debugger, image, results):
        ...
        if self.opt.tracking:
            debugger.add_img(self.pre_image_ori if self.pre_image_ori is not None else image, img_id="previous")
        ...
        for j in range(len(results)):
            if results[j]["score"] > self.opt.vis_thresh:
                ...
                item = results[j]
                if "bbox" in item:
                    sc = item["score"] if self.opt.demo == "" or not ("tracking_id" in item) else item["tracking_id"]
                    sc = item["tracking_id"] if self.opt.show_track_color else sc

                    debugger.add_coco_bbox(item["bbox"], item["class"] - 1, sc, img_id="generic")

                if "tracking" in item:
                    debugger.add_arrow(item["ct"], item["tracking"], img_id="generic")

                tracking_id = item["tracking_id"] if "tracking_id" in item else -1
                if "tracking_id" in item and self.opt.demo == "" and not self.opt.show_track_color:
                    debugger.add_tracking_id(item["ct"], item["tracking_id"], img_id="generic")

                if (item["class"] in [1, 2]) and "hps" in item:
                    debugger.add_coco_hp(item["hps"], tracking_id=tracking_id, img_id="generic")

        if len(results) > 0 and "dep" in results[0] and "alpha" in results[0] and "dim" in results[0]:
            debugger.add_3d_detection(
                image
                if not self.opt.qualitative
                else cv2.resize(debugger.imgs["pred_hm"], (image.shape[1], image.shape[0])),
                False,
                results,
                self.this_calib,
            ...
            debugger.add_bird_view(results, vis_thresh=self.opt.vis_thresh, img_id="bird_pred", cnt=self.cnt)
