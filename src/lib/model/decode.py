...
def _update_kps_with_hm(kps, output, batch, num_joints, K, bboxes=None, scores=None):
    if "hm_hp" in output:
        ...
        mask = (mask > 0).float()
        kps_score = (1 - mask) * hm_score + mask * scores.unsqueeze(-1).expand(batch, num_joints, K, 1)  # bJK1
...
def generic_decode(output, K=100, opt=None):
    ...
    regression_heads = ["tracking", "dep", "rot", "dim", "amodel_offset", "nuscenes_att", "velocity"]
    ...
    if "hps" in output:
        ...
        kps, kps_score = _update_kps_with_hm(kps, output, batch, num_joints, K, bboxes, scores)
        ret["hps"] = kps
