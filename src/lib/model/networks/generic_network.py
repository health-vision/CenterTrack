...
class GenericNetwork(nn.Module):
    def __init__(self, num_layers, heads, head_convs, num_stacks=1, opt=None):
        ...
        print("Using generic model with backbone {} and neck {}".format(opt.backbone, opt.neck))
        ...
        for head in self.heads:
            ...
            if len(head_conv) > 0:
                ...
                conv = nn.Conv2d(
                    last_channel, head_conv[0], kernel_size=head_kernel, padding=head_kernel // 2, bias=True
                )
                convs = [conv]
