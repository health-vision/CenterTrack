...
class Debugger(object):
    ...
    def add_coco_bbox(self, bbox, cat, conf=1, show_txt=True, no_bbox=False, img_id="default"):
        ...
        if self.opt.only_show_dots:
            ...
            if self.opt.show_trace:
                if track_id in self.trace:
                    ...
                    t_pre = ct
                    for t in trace[::-1]:
                        cv2.circle(self.imgs[img_id], t, 6 - cnt * 2, c, -1, lineType=cv2.LINE_AA)
    ...
    def add_coco_hp(self, points, tracking_id=0, img_id="default"):
        ...
        for j, e in enumerate(self.edges):
            if points[e].min() > 0 and points[e, 0].max() < w and points[e, 1].max() < h:
                c = self.ec[j] if not self.opt.show_track_color else self.track_color[tracking_id]
                cv2.line(
                    self.imgs[img_id],
    ...
    def save_all_imgs(self, path="./cache/debug/", prefix="", genID=False):
        ...
        for i, v in self.imgs.items():
            if i in self.opt.save_imgs or self.opt.save_imgs == []:
                cv2.imwrite(path + "/{}{}{}.png".format(prefix, i, self.opt.save_img_suffix), v)
    ...
    def add_3d_detection(self, image_or_path, flipped, dets, calib, show_txt=False, vis_thresh=0.3, img_id="det"):
        if self.opt.only_show_dots:
            return
        ...
        for item in dets:
            if item["score"] > vis_thresh and "dim" in item and "loc" in item and "rot_y" in item:
                cl = (
                ...
                if loc[2] > 1:
                    ...
                    if self.opt.show_track_color or self.opt.qualitative:
                        bbox = [box_2d[:, 0].min(), box_2d[:, 1].min(), box_2d[:, 0].max(), box_2d[:, 1].max()]
                        sc = int(item["tracking_id"]) if self.opt.show_track_color else item["score"]
                        self.add_coco_bbox(
                            bbox,
                            item["class"] - 1,
                            sc,
                    ...
                    if self.opt.show_track_color:
                        self.add_arrow(
                            [(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2], item["tracking"], img_id=img_id
                        )
    ...
    def add_bird_view(self, dets, vis_thresh=0.3, img_id="bird", cnt=0):
        if self.opt.vis_gt_bev:
            bird_view = cv2.imread(self.opt.vis_gt_bev + "/{}bird_pred_gt.png".format(cnt))
        else:
