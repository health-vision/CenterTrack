...
def _expt2bbox(expt):
    ...
    bbox = [int(expt[:, 0].min()), int(expt[:, 1].min()), int(expt[:, 0].max()), int(expt[:, 1].max())]
    return bbox
...
def save_txt(txt_name, pts_cls):
    ...
    for i in range(len(pts_cls)):
        ret.append(np.array(pts_cls[i][:4], dtype=np.int32).reshape(8).tolist() + [pts_cls[i][4]])
...
def click(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if len(pts) == 4:
            ...
            cv2.rectangle(img, (bbox[0], bbox[1]), (bbox[2], bbox[3]), (255, 0, 255), 2, cv2.LINE_AA)
            pts = []
...
if __name__ == "__main__":
    ...
    while i < len(image_names):
        ...
        txt_name = os.path.join(args.save_path, image_name[: image_name.rfind(".")] + ".txt")
