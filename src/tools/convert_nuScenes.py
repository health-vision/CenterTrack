...
def main():
    ...
    for split in SPLITS:
                ...
                if sensor_name in USED_SENSOR:
                    ...
                    for box in boxes:
                        ...
                        amodel_center = project_to_image(
                            np.array(
                                [box.center[0], box.center[1] - box.wlh[2] / 2, box.center[2]], np.float32
                            ).reshape(1, 3),
                            calib,
                        )[0].tolist()
                        ...
                        attributes = [nusc.get("attribute", att_token)["name"] for att_token in attribute_tokens]
                        ...
                        vel = np.dot(
                            np.linalg.inv(trans_matrix), np.array([vel[0], vel[1], vel[2], 0], np.float32)
                        ).tolist()
                        ...
                        alpha = _rot_y2alpha(
                            yaw, (bbox[0] + bbox[2]) / 2, camera_intrinsic[0, 2], camera_intrinsic[0, 0]
                        )
                    ...
                    for i in range(len(anns)):
                        ...
                        for j in range(len(anns)):
                            if anns[i]["depth"] - min(anns[i]["dim"]) / 2 > anns[j]["depth"] + max(
                                anns[j]["dim"]
                            ) / 2 and _bbox_inside(anns[i]["bbox"], anns[j]["bbox"]):
                                vis = False
                                break
                        if vis:
        ...
        print("{} {} images {} boxes".format(split, len(ret["images"]), len(ret["annotations"])))
